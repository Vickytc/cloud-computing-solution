...
Description: Main test function in 4th-iteration.
Authors:
    Qingze Wang (1528654)
    Ze Pang (955698) 
Usage:
...
class TestEnd2End(unittest.TestCase):
    def test_setUp(self):
        self.assertEqual(test_request.delete('/tstwipe4').status_code, 200)
        time.sleep(5)
    ...
    def test_mtd(self):
        ...
        self.assertEqual(test_request.put('/tstmtd4/1', data1).status_code, 201)
        self.assertEqual(test_request.put('/tstmtd4/2', data2).status_code, 201)

        time.sleep(1)
        r = test_request.get('/tstmtd4/1')
        self.assertEqual(r.status_code, 200)
        o = r.json()['_source']
        self.assertEqual(o['content'], 'test example 1')

        r = test_request.get('/tstmtd4/2')
        self.assertEqual(r.status_code, 200)
        o = r.json()['_source']
        self.assertEqual(o['content'], 'test example 2')

        r = test_request.put('/tstmtd4/1', data3)
        self.assertEqual(r.status_code, 200)
        r = test_request.get('/tstmtd4/1')
        o = r.json()['_source']
        self.assertEqual(r.status_code, 200)
        ...
        self.assertEqual(test_request.get('/tstmtd4/999').status_code, 404)

        self.assertEqual(test_request.delete('/tstmtd4/1').status_code, 200)
        self.assertEqual(test_request.delete('/tstmtd4/2').status_code, 200)
