"""
Script Name: mastodonharvester2.py
Description: Harvest data from Mastodon (mastodon.au server).
Authors:
    Jiajun Li (1132688)
    Qingze Wang (1528654)
    Ze Pang (955698) 
...
class BufferedStreamListener(StreamListener):
    ...
    def on_update(self, status: Dict[str, Any]) -> None:
        ...
        if status["language"]=="en":
            status["server"] = config('MTD_SERVER_2')
...
def main_stream() -> None:
    ...
    m = Mastodon(
            api_base_url = f"https://{config('MTD_SERVER_2')}",
            access_token = config('MTD_TOKEN_2'),
        )
    try:
